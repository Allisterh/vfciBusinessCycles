test_that("Replicated Classical VAR IRFs", {
  ...
  df <-
    fread(here::here("./data/bca-replication/replicated_bca_classical_VAR_IRF.csv")) |>
    _[, .(h, response, irf, version)] |>
    dt_pivot_wider(names_from = version, values_from = irf)
...
test_that("Replicated Bayesian VAR IRFs", {
  ...
  df <-
    fread(here::here("./data/bca-replication/replicated_bca_bayesian_VAR_IRF_bvartools.csv")) |>
    _[, .(model, version, h, response, median)]
...
test_that("Replicated BCA Minnesota Priors", {
  ...
  bcadata <- fread(here::here("./data-raw/bca_replication_data.csv")) |>
