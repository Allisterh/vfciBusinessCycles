...
mv_list <- list(
  max_share_u = id_fevdfd(fit_var(x[, -c(fin_cols), with = FALSE], lags = 2), "unemployment", bc_freqs),
  het_reg_c_fin = id_linear_het_reg(v, "output", hetreg_horizon = 10, x2 = fin_cols, extra_data = x[, ..fin_cols], method = "mriv", sign = "neg"),
  het_reg_c_macro = id_linear_het_reg(v, "output", het_reg_lags = 0:4, hetreg_horizon = 10, sign = "neg"),
  chol_vfci = id_ordered_chol(reorder_var(v2, "vfci_neg")),
  #chol_vfci_last = id_ordered_chol(v2),
  max_share_vfci = id_fevdfd(v2, "vfci_neg", bc_freqs, sign = "pos")
)
...
colors <- c(
  ...
  het_reg_c_fin = "lightblue",
  het_reg_c_macro = "steelblue",
  chol_vfci = "goldenrod",
...
int_vfci_fin <- mv_list$het_reg_c_fin$het_reg$dt[variable == "output", .(t, int_vfci_fin = log_var_fitted)]
int_vfci_macro <- mv_list$het_reg_c_macro$het_reg$dt[variable == "output", .(t, int_vfci_macro = log_var_fitted)]
