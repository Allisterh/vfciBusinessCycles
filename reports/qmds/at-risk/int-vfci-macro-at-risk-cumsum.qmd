...
hetreg_dt <-
  horizons |>
  purrr::set_names() |>
  purrr::map(~ {fit_het_reg_from_var(v, hetreg_horizon = .x, annualize_factor = 4, cumsum = TRUE)$dt}) |>
...
lms <-
  ...
  _[horizon %in% horizons] |>
  _[variable == "output"] |>
  split(f = dt[variable == "output" & horizon %in% horizons & date != "2020-04-01", horizon]) |>
  purrr::map(~ {
...
dt |>
  ...
  _[, at_risk := fitted + coef * exp(log_var_fitted / 2)] |>
  _[, not_at_risk := fitted - coef * exp(log_var_fitted / 2)] |>
  ggplot(aes(
    x = date,
    y = fitted
  )) +
