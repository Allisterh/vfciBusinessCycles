...
x <- data[, -"vfci"] |> copy()
v <- fit_var(x[, -c("t", add_cols), with = FALSE], lags = lags)

all_variables <- names(x[, -"date"]) |> set_names()

hr <- fit_het_reg_from_var(v, hetreg_horizon = 10, x2 = add_cols, extra_data = data[, ..add_cols])

hr_list <- purrr::map(1:30, ~ fit_het_reg_from_var(v, hetreg_horizon = .x, x2 = add_cols, extra_data = data[, ..add_cols])) 
...
comp_dt |>
  ...
  _[target_variable.x == "consumption"] |>
...
corr_dt |>
  ...
  labs(x = "Internal VFCI Horizon", y = "External VFCI Horizon")
...
comp_dt |>
  ...
  geom_line(aes(y = -1 * ext_vfci, color = "ext_vfci"))
```