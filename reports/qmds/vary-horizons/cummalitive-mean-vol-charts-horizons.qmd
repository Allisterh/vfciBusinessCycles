...
hetreg_list <- purrr::map(1:30, ~ fit_het_reg_from_var(v, hetreg_horizon = .x, annualize_factor = 4, cumsum = TRUE))
...
data[variable == "output" & horizon == 4] |> tail(20)
...
#| fig-height: 8
#| out-width: 100%
#| fig-cap: "Mean-Vol slopes by horizon"

data[!is.na(fitted), .(
  slope = coef(lm(data = .SD, "fitted ~ exp(log_var_fitted/2)"))[[2]]
  ), by = .(variable, horizon)
] |>
  ggplot(aes(
    x = horizon,
