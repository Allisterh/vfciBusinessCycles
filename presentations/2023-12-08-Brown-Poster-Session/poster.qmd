...
We show that financial conditions, particularly the VFCI which measures the price of risk, is also a part of the business cycle.
...
# Data
All data is measured quarterly and runs from `r zoo::as.yearqtr(min(vardata$date))` to `r zoo::as.yearqtr(as.Date("2017-01-01"))`.
Most of the time series used are standard macro series, pulled from the FRED by the Federal Reserve Bank of St. Louis.
An exception is the Volatility Financial Conditions Index (VFCI), introduced in  "The Market Price of Risk and Macro-Financial Dynamics" by Adrian, Duarte, and Iyer (WP).
...
p <- ggplot() +
  ...
  annotate(
    ...
    size = 4,
    hjust = 1,
    label = "NBER recessions shaded in gray."
  ) +
  scale_x_date(
    expand = c(0, 0),
...
![Raw VFCI Time Series](./figs/vfci.svg){#fig-raw-vfci}

:::

The VFCI can be interpreted as the **price of risk**.
It is consctucted using (1) asset returns and (2) forward consumption growth.
...
Pick one variable to target from $x_t$ (i.e. unemployment, $x_t^(u)$ or VFCI $x_t^(vfci)$).
...
We implement this identification scheme targeting unemployment and then again targeting VFCI.
...
We can run this same identification method for all variables in the VAR, identifying 11 separate shocks.
...
We will assume a recursive identifcation scheme (Choleskey), setting VFCI as the first variable in the VAR. This is equivalent to assuming that,
$$
...
This can be justified by VFCI being a financial variable measured and thus reacts to any new shock before the slowly moving macro variables.
...
![IRFs for Max FEV or Choleskey targeting VFCI](./figs/irf_choleskey_plot.svg){#fig-irf-maxfev-chol}

:::

It is clear in @fig-irf-maxfev-chol, that an innovation to the VFCI causes the same dynamics as the identified businness cycle shocks.
Given the assumptions for recursive identification, this shows that shocks to VFCI is a driver of the business cycle.

# Conclusion

Our work so far shows for certain that the financial conditions, as measured by the VFCI, are a part of business cycle.
