...
#' @param lags how many lags to include in the independent variables, defaults to 0
...
#' @param exlags boolean, should lags be included in the vfci calculation or excluded
...
est_vfci <- function(
  ...
  if (any(lags != 0)) {
    ...
    lag_cols <- purrr::map(cols_to_lag, ~ paste0(.x, "_lag", lags[lags != 0])) |> unlist()
    data[, (lag_cols) := shift(.SD, n = lags[lags != 0], type = "lag"), .SDcols = cols_to_lag]

    x <- c(x, lag_cols)
    het <- c(het, lag_cols)
    if (!exlags) vfci_vars <- c(vfci_vars, lag_cols)
  }
